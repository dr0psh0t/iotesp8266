...
void inithttp() {
  Serial.println("=== inithttp() ===");
	...
	if(WiFi.status()== WL_CONNECTED) {   //Check WiFi connection status
		//http.begin("http://192.168.1.189:8080/mcsa/IoTCheckWorkQueue");		//Specify destination for HTTP request
		http.begin("http://192.168.1.150:8080/joborder/IoTCheckWorkQueue");		//Specify destination for HTTP request
		http.addHeader("Content-Type", "application/x-www-form-urlencoded");	//Specify content-type header

		char params[128];
		snprintf(params, sizeof params, "%s%d", "madd=23AD12F3E44455&uId=2&h=3&fw=4&timestamp=5&mId=", mId);
