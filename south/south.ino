...
const char* ssid = "Wifi_South";
const char* password = "(---WifiSouth---)";
...
NTPClient timeClient(ntpUDP, "192.168.2.99", GMT_8);
...
void initWiFi() {
  ...
  IPAddress ip(192,168,2,hostId);
  IPAddress gateway(192,168,2,101);
  IPAddress subnet(255,255,255,0);
  IPAddress dns(192,168,2,101);
...
void inithttp() {
  Serial.println("=== inithttp() ===");
	...
	if(WiFi.status()== WL_CONNECTED) {   //Check WiFi connection status
		http.begin("http://192.168.2.99:8080/joborder/IoTCheckWorkQueue");    //Specify destination for HTTP request
